...
class MambaSequenceClassifierMemoryCNN(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.tokenizer = DownsamplingCNNTokenizer(d_model=d_model, downsample_factor=downsample_factor)
        self.layers = nn.ModuleList([
            MambaBlock(d_model=d_model, d_state=d_state, d_conv=d_conv, expand=expand)
            for _ in range(n_layers)
        ])
...
class MambaSequenceClassifierDualCNN(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.tokenizerFast = DownsamplingCNNTokenizer(d_model=d_model, downsample_factor=4, kernel_size=3)
        self.tokenizerSlow = DownsamplingCNNTokenizer(d_model=d_model, downsample_factor=4, kernel_size=9)
