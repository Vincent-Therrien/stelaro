...
def get_taxon_name(level: int, tax_id: str, lineages: dict) -> str:
    """Obtain the names defining a taxon at a specific taxonomic rank.

    Args:
        level: Taxonomic depth (0 = generic, 7 = species)
        tax_id: NCBI taxonomic identifier.
        lineages: Lineages as generated by the function `ncbi.resolve_taxonomy`.

    Returns: Taxon name.
    """
    if tax_id not in lineages:
        print(f"Unknown taxonomic identifier: `{tax_id}`.")
        return None
    if level == 0:
        for lineage in lineages[tax_id]:
            _, name, rank = lineage
            if rank == "domain":
                return name
        for lineage in lineages[tax_id]:
            ...
            if rank == "realm":
                return "Viruses"
    elif level < len(Taxonomy.RANKS):
        target = Taxonomy.RANKS[level]
        for lineage in lineages[tax_id]:
            _, name, rank = lineage
            if rank == target:
                return name
    else:
        raise RuntimeError(f"`{level}` is an invalid taxonomic level.")
    return None


def get_mappings(
        taxonomic_ids: tuple[str],
        levels: list[int],
        lineages: dict,
        ) -> dict:
    """Generate taxonomic mappings from lineages.

    Args:
        taxonomic_ids: NCBI taxonomic identifiers.
        levels: Taxonomic levels (e.g. `[0, 1]` for domain and phylum).
        lineages: Lineages as generated by the function `ncbi.resolve_taxonomy`.

    Returns: Dictionary whose keys indicate taxa
        (e.g. `('Eukaryota', 'Evosea')`) and values indicate the number of
        taxa inside of the
    """
    taxa = {}
    total = 0
    retained = 0
    for identifier in taxonomic_ids:
        total += 1
        mapping = []
        for level in levels:
            mapping.append(get_taxon_name(level, identifier, lineages))
        mapping = tuple(mapping)
        if None in mapping:
            continue
        retained += 1
        if mapping in taxa:
            taxa[mapping].append(identifier)
        else:
            taxa[mapping] = [identifier]
    print(f"Mapped {retained} / {total} taxonomic identifiers.")
    return taxa
